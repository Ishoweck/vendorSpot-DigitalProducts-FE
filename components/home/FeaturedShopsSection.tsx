...
export default function FeaturedShopsSection() {
  ...
  const handleClick = (businessName: string) => {
    Cookies.set("selectedBusinessName", businessName, { expires: 1 }); // 1 day
  };
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 sm:gap-6">
          {vendors.map((vendor: any, index: number) => (
            <div
              key={vendor._id}
              onClick={() => handleClick(vendor.businessName)}
              className={`cursor-pointer ${
                index >= 2 ? "hidden sm:block" : ""
              }`}
            >
              <Link href={`/vendorDetails`}>
                <div
                ...
                >
                  <div className="relative">
                    <Image
                      src={vendor.image || "/images/shop-placeholder.png"}
